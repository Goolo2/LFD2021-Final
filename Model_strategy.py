...
class Transformer(nn.Module):
    def __init__(self,  trg_vocab, d_model, N, heads, dropout, 图向量尺寸=6*6*2048):
        super().__init__()
        self.图转 = MLP(图向量尺寸, d_model)

        self.decoder = Decoder(trg_vocab, d_model, N, heads, dropout)
        self.outX = MLP(d_model, trg_vocab)

        self.评价 = MLP(d_model, 1)
...
class Agent:
    ...
    def supervised_rl(self, device, state, reward, action, action_prob, critic):
        ...
        for t in range(len(reward_set) - 1):
            gamma = 1
            adv_value = 0
            gamma = self.优势估计参数G * self.泛化优势估计参数L
            count = 0
        ...
        for i in range(3):
            ...
            ac_tensor = T.tensor(action).to(device)
            ...
            dist = Categorical(dist)
            #熵损失 = torch.mean(分布.entropy())
            action_logprob = dist.log_prob(ac_tensor)

            total_reward = advantage_func_value  # + 价值
            action_loss = -total_reward * action_logprob
            action_loss = action_loss.mean()
            print(total_reward[10:20], action_logprob[:, 10:20].exp())
